...
function mergeRowLeft(row) {
    const result = [];
    let scoreDelta = 0;
    for(let i = 0; i < row.length; i++)if (row[i] === row[i + 1] && row[i] !== 0) {
        const value = row[i] * 2;
        result.push(value);
        scoreDelta += value;
        i++;
    } else result.push(row[i]);
    return {
...
function setColumn(board, index, column) {
    for(let row = 0; row < 4; row++)board[row][index] = column[row];
}
class Game {
    ...
    moveUp() {
        ...
        for(let col = 0; col < 4; col++){
            ...
            const { row: mergedColumn, scoreDelta } = mergeRowLeft(slideRowLeft(column));
    ...
    moveDown() {
        ...
        for(let col = 0; col < 4; col++){
            ...
            const { row: mergedColumn, scoreDelta } = mergeRowLeft(slideRowLeft(reversedColumn));
...
function render() {
    ...
    if (gameStatus === "won") messageWin.classList.remove("hidden");
    if (gameStatus === "lost") messageLose.classList.remove("hidden");
}
